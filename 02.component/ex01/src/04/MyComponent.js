...
export default function MyComponent({ props01, props02, props03, props04, props05, props06, props07, props08, props09 }) {
    ...
    return (
        <Fragment>
            ...
            <span>props01: { props01 ? props01 : '--- not set ---' } </span><br/>
            <span>props02: { props02 ? props02 : '--- not set ---' } </span><br/>
            <span>props03: { props03 ? `${props03}` : '--- not set ---' } </span><br/>
            <span>props04: { props04 ? props04.no : '--- not set ---' } </span><br/>
            <span>props05: { props05 ? props05.map((e, index) => <b key={index}>{e}{' '}</b>) : '--- not set ---' } </span><br/>
            <span>props06: { props06 ? props06() : '--- not set ---' } </span><br/>
            <span>props07: { props07 ? props07 : '--- not set ---' } </span><br/>
            <span>props08: { props08 ? props08.map((e, index) => e ? <b key={index}>{'true'}{' '}</b> : <b key={index}>{'false'}{' '}</b> ) : '--- not set ---' } </span><br/>
            <span>props09: { 
                            props09 ? 
                                <div>
                                    <h3>{props09.no}</h3>
